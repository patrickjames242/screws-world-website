...
export default function LogInScreen(props: {
  authTokenHandler: (authToken: string) => void;
}) {
  ...
  function respondToOnSubmit(formEvent: React.FormEvent<HTMLFormElement>) {
    ...
    if (passwordText === "" || usernameText === "") {
      setErrorMessage(
        "You must enter both a username and a password to log in."
      );
      return;
    }
  ...
  useEffect(() => {
    function setLoginScreenMinWidth() {
      if (
        loginScreenRef.current == null ||
        verticallyCenteredContentRef.current == null
      ) {
        return;
      }

      const newAlignContentValue =
        loginScreenRef.current.clientHeight <
        verticallyCenteredContentRef.current.offsetHeight
          ? "start"
          : "center";
  ...
  return (
    <form
      ...
      onSubmit={respondToOnSubmit}
    >
      <div
        ref={verticallyCenteredContentRef}
