...
export default function ContactUs() {
  ...
  let [requestResult, setRequestResult] =
    useState<Optional<EmailRequestResult>>(null);

  const areThereChanges = (() => {
    return [email, name, subject, description].some((x) => x.trim().length > 0);
  })();

  useBlockHistoryWhileMounted(
    "If you leave this page, all the information you have entered so far will be lost",
    areThereChanges
  );
  ...
  const pageHeaderProps: PageHeaderProps = useRef({
    ...
    subtitle:
      "Please fill out the form below, email us, or call us, and one of our employees will be in touch with you shortly.",
  }).current;
  ...
  function respondToFormSubmission(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (
      [trimmedEmail, trimmedName, trimmedSubject, trimmedDescription].some(
        (x) => x === ""
      )
    ) {
      setRequestResult({
      ...
      <div className="body-content-holder">
        <form className="text-fields" onSubmit={respondToFormSubmission}>
          ...
          {(() => {
            ...
            const className =
              requestResult?.type === EmailRequestResultType.SUCCESS
                ? "positive"
                : "";
            return (
              <ErrorMessageBox
        ...
        <div className="contact-methods-holder">
          {[
            ["Email", SCREWS_WORLD_EMAIL, "mailto:" + SCREWS_WORLD_EMAIL],
            ["Phone", SCREWS_WORLD_NUMBER, "tel:" + SCREWS_WORLD_NUMBER],
          ].map((x, i) => {
            return (
              <a className="contact-method" key={i} href={x[2]}>
