...
import CustomInput, {
  CustomInputChildParams,
  CustomInputProps,
} from "random-components/CustomInputs/CustomInput";
...
export interface ProductItemImageSelectorProps
  extends CustomInputProps<OptionalDatabaseValue<File>> {
...
export default function ProductItemImageSelector(
  props: ProductItemImageSelectorProps & {
    imageFitModeProps: ProductImageSelectorImageFitModeProps;
  }
) {
  ...
  function respondToFileInputValueDidChange(
    ...
    compressImageFileIfNeeded(newValue)
      ...
      .catch((error) => {
        console.error(
          "something went wrong when trying to compress the image",
          error.message
        );
      });
  }
  ...
  return (
    <div
      className={["ProductItemImageSelector", props.className ?? ""].join(" ")}
    >
          ...
          return (
            ...
            >
              <ProductItemImageView
                imageSource={imageViewValue}
                imageContentFitMode={props.imageFitModeProps.value}
              />
            </div>
          );
        }}
      ...
      <input
        ...
        type="file"
        accept="image/*"
        onChange={respondToFileInputValueDidChange}
      />
      ...
      <div className="bottom-image-controls">
        ...
        <EditImageButtons
          ...
          onRemoveButtonClick={respondToRemoveButtonClicked}
          onSelectImageButtonClick={respondToSelectButtonClicked}
        />
      </div>
    </div>
  );
...
function FitOrFillChooser(props: {
  value: ProductImageContentFitMode;
  onChange: (newValue: ProductImageContentFitMode) => void;
}) {
  ...
  return (
    <div className="FitOrFillChooser">
      <RadioButtonOption
...
function EditImageButtons(props: {
  ...
  function respondToSelectImageButtonClicked(
  ...
  return (
    <div style={props.style} className="EditImageButtons">
      <button
        disabled={shouldButtonsBeDisabled}
        className="select-image-button"
        onClick={respondToSelectImageButtonClicked}
      >
        Select Image
      </button>
      {(() => {
        if (props.shouldShowRemoveButton) {
          return (
            <button
              ...
              onClick={respondToRemoveButtonClicked}
            >
              Remove
            </button>
          );
