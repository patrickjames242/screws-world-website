...
} from "App/MainSiteInterface/Products/ProductsDataHelpers";
import { useProductsInfoContextValue } from "App/MainSiteInterface/Products/ProductsUIHelpers";
import CustomSelect, {
  CustomSelectChild,
} from "random-components/CustomInputs/CustomSelect/CustomSelect";
...
interface ProductItemParentCategorySelectorProps
  extends CustomInputProps<OptionalDatabaseValue<number>> {
  itemBeingEdited: Optional<ProductDataObject>;
}

export default function ProductItemParentCategorySelector(
  props: ProductItemParentCategorySelectorProps
) {
  ...
  const customSelectChildren = useMemo(() => {
    ...
    allCategories.forEach((x) => {
      if (x.id.databaseID === props.itemBeingEdited?.id.databaseID) {
        return;
      }
      ...
      if (
        isProductCategory(props.itemBeingEdited) &&
        doesProductCategoryRecursivelyContainItem(x, props.itemBeingEdited)
      ) {
        return;
      }

      const title = (function getRecursiveTitleFor(
    ...
    children = children.sort((x1, x2) =>
      x1.stringValue.localeCompare(x2.stringValue)
    );
    children.unshift({
      uniqueID: String(NULL_PARENT_CATEGORY_ID),
      stringValue: "None (should be top level)",
    });
    return children;
  ...
  return (
    <CustomSelect
      {...props}
      value={value}
      placeholderText="What is the parent category?"
      onValueChange={respondToValueDidChange}
    >
