...
export interface ProductItemTypeSelectorProps
  extends CustomInputProps<RequiredDatabaseValue<ProductDataType>> {
  itemBeingEdited: Optional<ProductDataObject>;
}

export default function ProductItemTypeSelector(
  props: ProductItemTypeSelectorProps
) {
        ...
        if (props.itemBeingEdited == null) {
          return (
            <CustomSelect
              {...props}
              value={value}
              placeholderText="Is this a product or category?"
              onValueChange={respondToItemTypeDidChange}
            >
              {[ProductDataType.Product, ProductDataType.ProductCategory].map(
                (x) => {
