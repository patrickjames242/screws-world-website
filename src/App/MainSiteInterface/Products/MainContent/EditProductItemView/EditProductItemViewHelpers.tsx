...
function getDefaultParentCategoryValueFromLocation(
  location: Location
...
export function getDefaultUpdatePropertyStates(
  props: EditProductItemViewProps,
  currentLocation: Location
): StateProps {
  const defaultParent =
    getDefaultParentCategoryValueFromLocation(currentLocation);
...
export function getAreThereChangesToBeSavedValue(
  props: EditProductItemViewProps,
  ...
  const imageContentFitModeHasBeenChanged = (() => {
    const oldValue =
      props.itemToEdit?.imageContentFitMode ??
      ProductItemContentFitMode_Helpers.default;
  ...
  const allChanges = [
    ...
    productDataTypeIsSelected,
    parentCategoryIDIsChanged,
    imageHasBeenChanged,
    imageContentFitModeHasBeenChanged,
  ];
...
export function getAPIUpdateObjectFromState(
  props: EditProductItemViewProps,
  stateProps: StateProps
): ProductItemProps & { fetchItemType: FetchItemType } {
  const noValueProvidedError = (fieldTitle: string) => {
    return new Error(
      `The ${fieldTitle} field is required, but you have not provided a value for it.`
    );
  };
  ...
  return {
    title: (() => {
      ...
      const titleValueIsNotValid =
        typeof trimmedTitle_stateProps != "string" ||
        trimmedTitle_stateProps === "";
      ...
      if (props.itemToEdit == null) {
        ...
        return trimmedTitle_stateProps === props.itemToEdit.name
          ? undefined
          : trimmedTitle_stateProps;
      }
    })(),
    ...
    imageContentFitMode: (() => {
      if (props.itemToEdit == null) {
        ...
        props.itemToEdit.imageContentFitMode !== stateProps.imageContentFitMode
      ) {
        return stateProps.imageContentFitMode;
      } else {
        return undefined;
