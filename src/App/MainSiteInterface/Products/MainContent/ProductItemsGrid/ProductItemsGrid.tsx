...
export default function ProductItemsGrid(props: {
  products: ProductDataObject[];
}) {
  return (
    <div className="ProductItemsGrid">
      {props.products.map((x) => {
        return (
          <ProductOrCategoryItem dataObject={x} key={x.id.stringVersion} />
        );
      })}
    </div>
  );
...
function ProductOrCategoryItem(props: { dataObject: ProductDataObject }) {
  ...
  return (
    <Link to={path} className="ProductOrCategoryItem">
      <div className="background-view" />
      <div className="content-box">
        <div className="image-box">
          <ProductItemImageView
            imageSource={props.dataObject.imageURL}
            imageContentFitMode={props.dataObject.imageContentFitMode}
          />
          ...
          <div className="text-box">
            ...
            {(() => {
              if ((props.dataObject.description?.trim() ?? "") !== "") {
                return (
                  <p className="description">{props.dataObject.description}</p>
                );
              }
            })()}
          </div>
