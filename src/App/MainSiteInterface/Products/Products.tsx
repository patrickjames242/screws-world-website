...
import {
  ProductsInfoContext,
  computeProductsInfoContextValueFromFetchResult,
} from "./ProductsUIHelpers";

import MainContent from "./MainContent/MainContent";
import DetatchedSideBar, {
  DetatchedSideBarFunctionsRef,
} from "./SideBar/DetachedSideBar/DetachedSideBar";
...
export default function Products() {
  ...
  const [productsObjectsManager, setProductsObjectsManager] =
    useState<Optional<ProductsDataObjectsManager>>(null);
  const [productsInfoFetchError, setProductsInfoFetchError] =
    useState<Optional<Error>>(null);

  const contextProviderValue = useMemo(() => {
    return computeProductsInfoContextValueFromFetchResult(
      location.pathname,
      productsObjectsManager,
      productsInfoFetchError
    );
  ...
  useEffect(() => {
    ...
    startFetchingProductsDataTree()
      .then((result) => {
        if (shouldRespondToFetchResult === false) {
          return;
        }
    ...
    return () => {
      shouldRespondToFetchResult = false;
    };
  }, []);
  ...
  return (
    <ProductsInfoContext.Provider value={contextProviderValue}>
