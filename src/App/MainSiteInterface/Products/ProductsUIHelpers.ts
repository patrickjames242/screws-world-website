...
import {
  DashboardProductsRouteURLs,
  MainUIProductsRouteURLs,
  DashboardProductsRouteMatchPaths,
  MainUIProductsRouteMatchPaths,
} from "./ProductsRoutesInfo";
...
function getProductsPageSubjectForRoutePath(
  routePath: string,
  productsObjectsManager: ProductsDataObjectsManager
): ProductsPageSubject {
  ...
  return (() => {
    switch (match?.path) {
      case DashboardProductsRouteMatchPaths.editProductItem:
        ...
        return new ProductsPageSubject(
          ProductsPageSubjectType.EDIT_ITEM,
          productItem
        );

      case DashboardProductsRouteMatchPaths.createProductItem:
        return new ProductsPageSubject(
          ProductsPageSubjectType.CREATE_NEW,
          null
        );
      ...
      case MainUIProductsRouteMatchPaths.productDetailsView:
        if (isProduct(productItem)) {
          return new ProductsPageSubject(
            ProductsPageSubjectType.PRODUCT,
            productItem
          );
        } else if (isProductCategory(productItem)) {
          return new ProductsPageSubject(
            ProductsPageSubjectType.CATEGORY,
            productItem
          );
        } else {
          break;
      ...
      case MainUIProductsRouteMatchPaths.root:
        return new ProductsPageSubject(
          ProductsPageSubjectType.INTRO_PAGE,
          productsObjectsManager.getTopLevelItems()
        );
    }
...
export function computeProductsInfoContextValueFromFetchResult(
  routePath: string,
  objectsManager: Optional<ProductsDataObjectsManager>,
  fetchError: Optional<Error>
): ProductsInfoContextValue {
  if (objectsManager) {
    return {
...
export const ProductsInfoContext =
  React.createContext<Optional<ProductsInfoContextValue>>(null);

export function useProductsInfoContextValue(): ProductsInfoContextValue {
  const value = React.useContext(ProductsInfoContext);
  if (!value) {
    throw new Error(
      "tried to access the projects page subject outside of the Products component. This is not allowed."
    );
  }
  return value;
}
...
export function useCurrentProductDetailsViewItem(): Optional<ProductDataObject> {
  ...
  if (
    subject.type !== ProductsPageSubjectType.CATEGORY &&
...
export function useToURLForProductItem(
  productsItem: ProductDataObject
): string {
  const isDashboard = useIsDashboard();
  return isDashboard
    ? DashboardProductsRouteURLs.productDetailsView(productsItem.id)
    : MainUIProductsRouteURLs.productDetailsView(productsItem.id);
}
