...
import {
  useAlertFunctionality,
  CustomAlertInfo,
  CustomAlertButtonInfo,
  CustomAlertButtonType,
  CustomAlertController,
  CustomAlertButtonController,
  CustomAlertTextFieldInfo,
  CustomAlertTextFieldController,
} from "random-components/CustomAlert/CustomAlert";
...
import {
  useDashboardInfo,
  useRequestsRequiringAuth,
} from "App/Dashboard/DashboardUIHelpers";
import {
  isProduct,
  isProductCategory,
  ProductDataObject,
...
export default function TopActionButtonsView() {
  ...
  function respondToLogOutButtonClicked() {
    ...
    const cancelButton = new CustomAlertButtonInfo(
      "Cancel",
      dismissAlert,
      CustomAlertButtonType.SECONDARY
    );

    const logOutButton = new CustomAlertButtonInfo(
      "Log Out",
      () => {
  ...
  const createNewButtonShouldBeDisplayed =
    currentSubject != null &&
    currentSubject.type !== ProductsPageSubjectType.CREATE_NEW;
  ...
  return (
    <div className={className}>
      ...
      <TopActionButton
        svgIcon={logOutIcon}
        title="log out"
        onClick={respondToLogOutButtonClicked}
      ...
      {respondToDeleteButtonClicked ? (
        <TopActionButton
          ...
          onClick={respondToDeleteButtonClicked}
          isDestructive
        />
      ) : null}
    </div>
...
function TopActionButton(props: {
  ...
  const className =
    "TopActionButton" +
    (" " + props.className ?? "") +
    (props.isDestructive ?? false ? " destructive" : "");

  if (props.link) {
    return (
      <Link to={props.link} className={className} onClick={props.onClick}>
        {props.svgIcon}
      </Link>
    );
  } else {
...
function useDeleteAlertInfo(): Optional<CustomAlertInfo> {
  const currentlyDisplayedItem =
    useCurrentProductsPageSubject()?.associatedData;
  const history = useHistory();
  const apiRequests = useRequestsRequiringAuth();

  if (
    isProductCategory(currentlyDisplayedItem) === false &&
    isProduct(currentlyDisplayedItem) === false
  ) {
    return null;
  }
  ...
  const willChildrenBeDeleted =
    isProductCategory(currentlyDisplayedItem) &&
    currentlyDisplayedItem.children.length >= 1;
  ...
  const deleteButton = new CustomAlertButtonInfo(
    ...
    (dismiss) => {
      ...
      const urlToRedirectTo = (() => {
        ...
        return id == null
          ? DashboardProductsRouteURLs.root
          : DashboardProductsRouteURLs.productDetailsView(id);
      })();

      apiRequests
        .deleteItem(
          itemType,
          (currentlyDisplayedItem as ProductDataObject).id.databaseID
        )
        .finally(() => {
  ...
  const textFieldInfo: Optional<CustomAlertTextFieldInfo> =
    willChildrenBeDeleted
      ? {
          onMount: (c) => {
  ...
  const description = (() => {
    if (willChildrenBeDeleted) {
      return (
        "If you delete this category, ALL products and categories under it will also be deleted. Are you sure you want to continue?\n\nPlease type " +
        confirmationMessage +
        " to confirm."
      );
    } else {
