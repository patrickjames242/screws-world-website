...
const ScreenDimmerFunctionsContext = React.createContext<Optional<ScreenDimmerFunctions>>(null);

export function useScreenDimmerFunctions(): ScreenDimmerFunctions {
    const functions = useContext(ScreenDimmerFunctionsContext);
    if (!functions) {
        throw new Error("you tried to access the screen dimmer functions outside of a screen dimmer context provider. This is not allowed.")
    }
    return functions;
}
