...
const AlertContext =
  React.createContext<Optional<AlertProviderFunctions>>(null);
...
export default function AlertProvider(props: { children: React.ReactNode }) {
        ...
        if (prevAlerts.some((a) => isEqual(a, info))) {
          prevAlerts.forEach((x, i) => {
            ...
            if (x.shouldReplaceInfo?.(info) === true) {
              ...
              throw new Error(
                "tried to present an alert that is already being displayed"
              );
            }
          });
        } else {
  ...
  function removeAlertForAlertInfo(info: CustomAlertInfo) {
    setAlerts((prevAlerts) => {
      return [
        ...prevAlerts.filter(
          (a) => a !== info && a.uniqueKey !== info.uniqueKey
        ),
      ];
    });
  }
  ...
  return (
    <AlertContext.Provider value={alertProviderFunctions}>
      {props.children}
      {alerts.map((a) => {
        return (
          <CustomAlert
            key={a.uniqueKey}
            {...a}
            onAlertIsFinishedDismissing={() => removeAlertForAlertInfo(a)}
          />
        );
      })}
...
function CustomAlert(
  props: CustomAlertInfo & { onAlertIsFinishedDismissing: () => void }
) {
  ...
  function notifyThatAnimationHasFinished() {
    if (
      backgroundIsDismissed.current === false ||
      centerContentIsDismissed.current === false ||
      shouldBePresented
    ) {
      return;
    }
  ...
  const centerContentStyle = useSpring({
    ...
    to: {
      opacity: shouldBePresented ? 1 : 0,
      transform: shouldBePresented
        ? "translateY(0px) rotate(0deg)"
        : "translateY(20rem) rotate(20deg)",
    },
    onRest: () => {
      if (shouldBePresented === false) {
  ...
  return (
    <div className="CustomAlert">
      ...
      <animated.div
        ...
        className="vertically-centered-box"
      >
        <div className="horizontally-centered-box">
          ...
          {(() => {
            ...
            return (
              <CustomTextField
                ...
                placeholderText={
                  props.textFieldInfo?.placeholderText ?? "Type here..."
                }
              />
            );
          })()}
          ...
          <div className="button-box">
            {(() => {
              return [props.leftButtonInfo, props.rightButtonInfo].map(
                (x, i) => {
                  if (!x) {
                    return null;
                  }
                  return (
                    <CustomAlertButton
                      key={i}
                      {...x}
                      dismissAlertAction={dismissAlert}
                    />
                  );
                }
              );
...
function CustomAlertButton(
  props: CustomAlertButtonInfo & { dismissAlertAction: () => void }
) {
  ...
  const className = [
    ...
    (() => {
      switch (props.type) {
        case CustomAlertButtonType.PRIMARY:
          return "primary";
        case CustomAlertButtonType.PRIMARY_DESTRUCTIVE:
          return "primary-destructive";
        case CustomAlertButtonType.SECONDARY:
          return "secondary";
      }
    })(),
  ].join(" ");
