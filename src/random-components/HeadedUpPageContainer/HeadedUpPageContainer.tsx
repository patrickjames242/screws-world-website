...
import PageHeader, {
  PageHeaderProps,
} from "random-components/PageHeader/PageHeader";
...
export default function HeadedUpPageContainer(
  props: HeadedUpPageContainerProps
) {
  const contentRef = useRef<HTMLDivElement>(null);

  const maxContentWidth = props.stylingProps?.maxContentWidth ?? "60rem";

  const [maxContentWidthHasBeenSet, setMaxContentWidthHasBeenSet] =
    useState(false);
  ...
  useEffect(() => {
    ...
    contentRef.current.style.setProperty(
      "--HeadedUpPageContainer-max-content-width",
      maxContentWidth
    );
