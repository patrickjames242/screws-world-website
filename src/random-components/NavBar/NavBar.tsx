...
export default function NavBar() {
  ...
  const { toggleIsExpanded, springStyle } =
    useExpandCollapseFunctionality(navBarRef);

  return (
    <animated.nav ref={navBarRef} style={springStyle} className="NavBar">
      <div className="nav-bar-content">
        ...
        <div className="links-box">{getAllNavBarLinks()}</div>
        <button className="menu-icon-holder" onClick={toggleIsExpanded}>
          {menuIcon}
        </button>
      </div>

      <div className="narrow-links">{getAllNavBarLinks()}</div>
    </animated.nav>
  );
}

const wideNavBarLinksCutOffPoint = window.matchMedia(
  `(max-width: ${scssVariables.wideNavBarLinksCutOffPoint})`
);

function useExpandCollapseFunctionality(
  navBarRef: React.MutableRefObject<Optional<HTMLDivElement>>
) {
  const navBarHeight = scssVariables.navBarHeight;
  const expandedNarrowNavBoxHeight =
    scssVariables.totalNavBarHeightWhenExpanded;
  ...
  const _setExpanded = useCallback(
    (
      isExpanded: ((prevProp: boolean) => boolean) | boolean,
      isAnimated: boolean
    ) => {
      const updateIsAnimated = () =>
        (shouldAnimateNextExpandRef.current = isAnimated ?? true);
      ...
      if (typeof isExpanded === "boolean") {
        ...
        setIsExpanded((prevProp) => {
          const shouldExpand = (isExpanded as (prev: boolean) => boolean)(
            prevProp
          );
          updateIsAnimated();
...
function NavBarLink(props: {
  ...
  return (
    <NavLink className="NavBarLink" to={routePath} activeClassName="selected">
